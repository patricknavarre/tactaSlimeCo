...
export async function GET() {
  try {
    ...
    const COLLECTION_ID = process.env.SHOPIFY_COLLECTION_BEST_SELLERS;
    
    if (!COLLECTION_ID) {
      console.error('SHOPIFY_COLLECTION_BEST_SELLERS is not defined in environment variables');
    ...
    const products = response.data.collection.products.edges.map(({ node }) => ({
      ...
      salePrice: node.compareAtPriceRange.minVariantPrice.amount 
        ...
        : null,
      onSale: node.compareAtPriceRange.minVariantPrice.amount ? true : false,
    ...
    // await syncToMongoDB(products, 'best-sellers');

    return NextResponse.json(products);
  } catch (error) {
    console.error('Error fetching Best Sellers collection:', error);
