...
export default function Home() {
  ...
  const [content, setContent] = useState({
    home: {
      heroTitle: 'Discover the Magic',
      heroSubtitle: 'Sensory Squishables and Slime',
  ...
  useEffect(() => {
    const fetchContent = async () => {
      try {
        ...
        const response = await fetch(`/api/content?nocache=${Date.now()}`);
        ...
        if (data.success && data.content) {
          ...
          const modifiedContent = {
            ...
            home: {
              ...data.content.home,
              // Always use our local heroTitle value, don't override with DB value
              heroTitle: 'Discover the Magic',
              // DO NOT override the heroSubtitle - get it from the DB
          ...
          console.log('Hero subtitle from DB:', modifiedContent.home.heroSubtitle);
  ...
  // Function to manually refresh all content
  const refreshContent = () => {
    console.log('Manually refreshing content');
    setIsForceRefresh(prev => prev + 1);
  };

  // Add effect to refresh content when returning to the homepage
  useEffect(() => {
    if (pathname === '/') {
      refreshContent();
    }
  ...
  return (
    <Layout>
      ...
      <section className="relative min-h-[60vh] flex items-center overflow-hidden">
        ...
        <div className="absolute inset-0 bg-cover bg-center" style={{ backgroundImage: "url('/images/Hero_Background_Image_TactaSlime.jpg')" }}></div>
