...
function SaleItemsContent() {
  ...
  useEffect(() => {
    async function fetchSaleItems() {
      try {
        setLoading(true);
        console.log('Fetching sale items from Shopify...');
        const response = await fetch('/api/shopify/collections/sale-items');
        ...
        setError(`Failed to load sale items: ${error.message}`);
      } finally {
        setLoading(false);
      }
    }
  ...
  const handleQuickAdd = (product) => {
    ...
    console.log("Adding to cart from sale items:", productToAdd);
  ...
  const calculateDiscount = (price, salePrice) => {
    if (!price || !salePrice || price <= 0 || salePrice <= 0) return 0;
    const discount = ((salePrice - price) / salePrice) * 100;
    return Math.abs(Math.round(discount));
  };
  ...
  return (
    <div className="container-custom py-8">
      ...
      >
        <h1 className="text-4xl font-bold text-gray-900 mb-4">Sale Items</h1>
        <p className="text-lg text-gray-700 max-w-3xl mx-auto px-4">
          Great deals on our premium slimes! Limited-time offers with discounted prices on select items.
        </p>
      </motion.div>
      ...
      {loading ? (
        <div className="text-center py-16">
          ...
          <p className="text-lg text-gray-600">Loading sale items...</p>
        </div>
      ) : error ? (
        ...
        <div className="text-center py-16 bg-white rounded-lg shadow-sm">
          ...
          <h3 className="text-xl font-semibold text-gray-700 mb-2">No sale items found</h3>
          <p className="text-gray-500">
            There are no active sales at the moment. Check back soon for special discounts!
          </p>
          <Link href="/products">
            <motion.span 
                      ...
                      <div>
                        {product.onSale && product.salePrice ? (
                          <div className="flex items-center gap-2">
                            <span className="font-semibold text-lg text-red-500">${product.price.toFixed(2)}</span>
